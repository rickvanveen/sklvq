...
def test_relevance_correction():
    ...
    correction_matrix = np.identity(correction_vectors.shape[1]) - (correction_vectors.T.dot(correction_vectors))
    ...
    assert np.allclose(
        np.dot(model2.omega_, correction_vectors.T.dot(correction_vectors)), np.zeros(correction_vectors.shape[1])
    )
